...
"""
d_s and d3_s mean the pre-warm delay for stage 2 and stage 3, respectively
Return: 
"""
def getE2EandUtilDist(d_s: int, d3_s: int):
    ...
    for i, f1 in enumerate(F1_times): # execution time of function 1
        ...
        if (f1 <= (d + Init_f2)):
            ...
            util = 1 / (1 + ((f1 - (d + Init_f2)) / (f1 + Init_f2 + f2)) + (f1_f2 - (d3 + Init_f3)) / (
                        f1 + f2 + f3 + Init_f2 + Init_f3))
...
print("=========== Best Prewarming Delays: Split->Extract: " + str(
    round(d2_curr, 2)) + " seconds Extract->Classify: " + str(round(d3_curr, 2)) + " seconds")
